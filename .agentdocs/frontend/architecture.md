...
- **版本号（注入机制）**：页面与终端展示的产品版本（mast/hero-meta/静态终端/页脚/动画头部共 5 处）**一律不得硬编码**。单一来源为主仓库 `kuraya/__init__.py` 的 `__version__`，构建/开发时经 `vite.config.js` 注入：HTML 侧用 `{{VERSION}}` 占位（`transformIndexHtml` 替换），JS 侧（main.js 动画品牌头）用 `define` 常量 `__KURAYA_VERSION__`。主仓库默认取 `../Kuraya` 相对路径，可用 `KURAYA_REPO` 覆盖；`KURAYA_VERSION` 可显式指定。主仓库缺失/无 `__version__` 时构建直接报错（不静默兜底假版本）。新增版本号展示位必须走注入，禁止手写。
