...
- **版本号（注入机制）**：页面与终端展示的产品版本（mast/hero-meta/静态终端/页脚/动画头部共 5 处）**一律不得硬编码**。单一来源为主仓库 `kuraya/__init__.py` 的 `__version__`，构建/开发时经 `vite.config.js` 注入：HTML 侧用 `{{VERSION}}` 占位（`transformIndexHtml` 替换），JS 侧（main.js 动画品牌头）用 `define` 常量 `__KURAYA_VERSION__`。解析顺序：`KURAYA_VERSION` 环境变量 > 主仓库 `__init__.py`（默认 `../Kuraya`，`KURAYA_REPO` 覆盖）> `public/version.txt`。托管平台构建（无主仓库，如 Cloudflare Pages）回退到 `public/version.txt`，该文件由 `.github/workflows/sync-version.yml` 在发版后自动同步：主仓库 `release.yml` 配置 `SITE_DEPLOY_TOKEN` secret 时发版即发 repository_dispatch 即时触发；否则每 6 小时定时轮询主仓库兜底；也可在 Actions 手动指定版本运行。主仓库与 version.txt 均缺失时构建直接报错（不静默兜底假版本）。`public/version.txt` 仅作托管构建的版本来源，正常流程只允许 workflow 写入，勿手动改。新增版本号展示位必须走注入，禁止手写。
